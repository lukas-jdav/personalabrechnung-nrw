...
customElements.define("cost-section", class extends HTMLElement {
	...
	appendRow(costKind) {
		...
		row.container.dataset.kindIndex = this.costKinds.indexOf(costKind)
	...
	getState() {
		const rows = []
		this.elements.rows.querySelectorAll(".row").forEach(rowEl => {
			const kindIndex = parseInt(rowEl.dataset.kindIndex)
			if (isNaN(kindIndex)) return
			const costItem = rowEl.querySelector("cost-item")
			const inputs = {}
			costItem?.shadowRoot?.querySelectorAll("input, select, textarea").forEach(el => {
				if (el.id) inputs[el.id] = el.type === "checkbox" ? el.checked : el.value
			})
			rows.push({ kindIndex, inputs })
		})
		return rows
	}

	setState(savedRows) {
		savedRows.forEach(({ kindIndex, inputs }) => {
			if (kindIndex < 0 || kindIndex >= this.costKinds.length) return
			this.appendRow(this.costKinds[kindIndex])
			const rowEls = this.elements.rows.querySelectorAll(".row")
			const costItem = rowEls[rowEls.length - 1]?.querySelector("cost-item")
			if (!costItem?.shadowRoot) return
			Object.entries(inputs).forEach(([id, value]) => {
				const el = costItem.shadowRoot.querySelector("#" + id)
				if (!el) return
				if (el.type === "checkbox") el.checked = value
				else el.value = value
			})
			const firstEl = costItem.shadowRoot.querySelector("input, select, textarea")
